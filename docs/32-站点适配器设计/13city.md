...
| NFO | 是（`nfo` file 字段） |
| 单种上行限速 | 100Mb/s（超速封禁） |
| H&R 模式 | 手动勾选（发布者决定），考察期 240h，做种≥24h 或分享率≥2.0 |
| 认领 | 最多 100 人/种，达标获 2 倍魔力 |
| 仅支持下载器 | qBittorrent、Transmission |
...
### 1.5 种子信息规范（来自 rules.php 2025 修订版）
...
```
例：`The Dark Knight 2008 PROPER 720p BluRay x264 - SiNNERS`

**电视剧**：
```
[英文名] 名称 [年份] S**E** [发布说明] 分辨率 来源 [音频/]视频编码-发布组名称
```
例：`Prison Break S04E01 PROPER 720p HDTV x264 - CTU`

**音轨**：
```
英文艺术家名 - 专辑名 [年份] [版本] [发布说明] 音频编码[-发布组名称]
```
例：`Enya - And Winter Came 2008 FLAC`

#### 副标题要求
- 不得含广告、求种/续种请求

#### 外部信息要求
- 电影、电视剧类种子，若有 IMDb 等外部链接需包含

#### 简介要求
- NFO 图需写入 NFO 文件，勿粘贴到简介
- 电影/电视剧/动漫：含海报/封面（如有）、截图/缩略图及链接、文件详细信息、演职员名单/剧情概要
- 体育节目：文字/截图/文件名等不得泄露比赛结果
- 音乐：含专辑封面、曲目列表
- PC 游戏：含海报/封面、截图/缩略图

#### 杂项要求
- 需正确选择资源类型、质量信息
- 管理员可编辑种子信息，上传者可修正错误，但不得改动管理员修改内容
- 种子信息不合规可能被删除
- 资源原始信息合规时，尽量沿用

---

## 二、发布页面表单字段分析（2026-04-22 Playwright 实际采集）

> 用户 ranfish 已登录，页面标题 `13City :: 发布 - Powered by NexusPHP`，表单共 48 个元素。
...
### 2.3 标签字段（`tags[4][]`）- 复选框（2026-04-22 实际采集）
