...
标签通过checkbox选择，字段名为 `tags[4][]`，共16个标签：

| ID | 标签名 | 审种脚本检测 | 说明 |
|----|--------|-------------|------|
| 1 | 禁转 | — | 标记禁止转载 |
| 2 | 首发 | — | 标记首发资源 |
| 4 | DIY | ✅ 交叉验证 | 与标题/副标题DIY关键词+文件结构交叉验证 |
| 5 | 国语 | ✅ 三方交叉 | 与MI音频语言+简介◎语言+副标题交叉验证 |
| 6 | 中字 | ✅ 交叉验证 | 与MI/DiscInfo Text字幕语言交叉验证 |
| 7 | HDR | ✅ 交叉验证 | 与MI/DiscInfo HDR信息交叉验证（五层递进检测） |
| 8 | 分集 | ✅ 结构校验 | 电视剧必需（与完结互斥） |
| 9 | 原创 | — | 标记原创资源 |
| 11 | 汉化 | — | 标记汉化资源 |
| 12 | 完结 | ✅ 结构校验 | 电视剧必需（与分集互斥），副标题含"完结"时需补充 |
| 13 | 短剧 | — | 短剧分类关联标签 |
| 14 | 杜比 | ✅ 交叉验证 | 与MI/DiscInfo Dolby Vision信息交叉验证 |
| 15 | 粤语 | — | 含粤语音轨标记 |
| 16 | 无损 | — | 无损音频标记 |
| 17 | 驻站 | — | 驻站制作组标记 |
| 18 | 纯享版 | ✅ 警告提示 | 仅警告提示人工检查 |

> **重要发现**：审种脚本第2608行的 `specialTags` 数组包含"原盘"，但**上传页实际标签列表中没有"原盘"标签**。原盘/DIY的检测逻辑（`checkYPTag`）是基于 DiscInfo存在性 + 文件列表(.mkv) + 标题关键词 来判定的，即使无此标签也可触发检测提示。

> **注意**：电影不允许包含"完结"(12)或"分集"(8)标签。
...
| 等级 | 退休待遇 |
|------|---------|
| 上传员(Uploaders) | 成为养老族(Retiree)：领工资次数≥12次，其余以进组要求为准 |

> 特殊情况（如原碟发布、0day更新等）可由管理组投票表决，须被认定为作出过重大及持久的贡献。
...
> 来源：用户提供站方种审脚本全文分析（zmpt-check-tool v2026.08.08.20.22 油猴 JS，/tmp/0 归档）。与 docs/38 共性矩阵的站特例部分。
> 脚本架构：三级输出 correct(√)/warn(!)/error(×) → 一键通过/拒绝按钮（审核弹窗 iframe 自动化）；核心特色为**转载来源声明校验（57 站官方映射表）+ 官种/驻站保护**。仅审核五类：电影/电视剧/纪录片/动漫/综艺，其它类型提示"请自行判断"后终止。

### 错误级（阻断通过）

| 判定条件 | 错误消息 |
|---------|---------|
| 官组名命中站点映射表（57 站，见下）但简介无声明关键词（见下） | [转载来源检查]：国内资源站 X 需要声明来源 |
| 简介无 MediaInfo 且无 DiscInfo（fieldset 解析） | [MediaInfo\|DiscInfo校验]：该简介缺少MediaInfo或者DiscInfo信息，请人工确认！ |
| 标题含中文/中文字符（含全角） | [种子标题校验]：种子标题包含中文或中文字符 |
| 分辨率 P 大写（`(480\|720\|1080\|2160\|4320)P`） | [种子标题校验]：种子标题分辨率P应改为小写p |
| 无标准分辨率词但含 4K | [种子标题校验]：种子标题4K应改为2160p |
| 标题空格数 <5 | [种子标题校验]：种子标题中空格数量少于5个，请检查种子标题格式 |
| HDR 非全大写（独立词 hdr/Hdr 等） | [种子标题校验]：种子标题 X 需要更换为HDR（全部大写） |
| 标题无年份（19xx/20xx） | [种子标题校验]：在主标题未找到有效的年份信息，请人工确认 |
| 年份在版本词（PROPER/REPACK/READ.NFO/RERIP）之后 | 年份 X 应在版本 Y 前面 |
| 标题（去组名段）无有效视频编码词（整词匹配：x264/x265/H.264/H.265/AVC/HEVC/VC-1/AVS2/AVS3/AV1/AVS+/H265/H264/MPEG-2） | [种子标题校验]：在主标题未找到有效的视频格式，请人工确认 |
| 选择视频格式 vs 解析不一致（标题词法+MI 细化）；或选 Other 但检测到格式 | [视频格式校验]：视频格式 X 与内容不一致，内容解析为 Y / 发布者选择了Other，但实际检测到视频格式 X |
| 主标题及媒体信息均缺失分辨率；或选择与实际不一致（MI 宽高→标准分辨率优先，标题末位词兜底） | [分辨率校验]：主标题及媒体信息中均缺失分辨率，需要补充 / 分辨率 X 与内容不一致，实际分辨率为 Y |
| 选择音频格式 vs MI/DiscInfo 全轨格式集合不一致；或选 Other 但标题+MI 均检测到 | [音频格式校验]：音频格式 X 与内容不一致，内容解析为 Y / 发布者选择了Other，但标题检测到音频格式 X |
| MI/DiscInfo 有中文字幕轨但未选「中字」；或选了但未检测到（ZM 官种豁免） | [中字标签校验]：发布者未选择中字标签，X检测到中文字幕，请补充 / 发布者选择了中字标签，X未检测到中文字幕，请检查是否有中文字幕截图 |
| MI/DiscInfo 有 Dolby Vision（HDR format/Format profile/Commercial name/Title 字段）vs「杜比」标签双向不符 | [杜比标签校验]：未选杜比但检测到杜比视界内容[X] / 选了杜比但未检测到杜比视界内容 |
| MI/DiscInfo 有 HDR（HDR10/HDR10 compatible/SMPTE ST 2086；DV P7/其他 DV；HDR Vivid/CUVA；PQ+BT.2020→HDR Unknown；**DV P5 不算 HDR**）vs「HDR」标签双向不符 | [HDR标签校验]：发布者未选择HDR标签，X已检测到[Y]，请补充 / 发布者选择了HDR标签，X未检测到HDR内容 |
| 选「国语」但 MI/DiscInfo/简介(语言/产地)/文件名均未检测到国语 | [国语标签检测]：国语标签已选择，简介、X和种子文件名均未检测到国语，请人工确认 |
| 未选「国语」但 MI/DiscInfo 音轨、简介语言产地、文件名携带国语 | [国语标签检测]：国语标签未选择，X携带国语，请补充国语标签 |
| 类型为电影但含「完结」或「分集」标签 | [电影标签检测]：电影不能包含X标签 |
| 「分集」+「完结」同时选择 | [标签互斥校验]：分集和完结标签为互斥标签，不能同时选择 |
| 电视剧（type=TV Series）无分集/完结标签；副标题含"完结"但未选完结标签 | [电视剧校验]：电视剧分集/完结标签缺失，需要补充标签 / 副标题携带完结字样，请人工检查是否需要补充完结标签 |
| 动漫：标题 SxxExx 或副标题"第N集/N-M集"应补分集；副标题"全N集"应补完结 | [动漫校验]：动漫包含分集内容，缺失分集/完结标签，建议补充分集标签 / 动漫包含完整季内容，缺失分集/完结标签，建议补充完结标签 |
| 「原盘」+「DIY」同时选择 | [原盘\|DIY检测]：原盘和 DIY 不能同时选择，请人工确认 |
| 疑似原盘 DIY（无 MI 有 DiscInfo，或文件列表无 .mkv，且标题含 blu-ray/bluray/diy/原盘 或副标题含 diy/原盘）但未选标签；或非原盘 DIY 但选了标签 | [原盘\|DIY检测]：种子疑似为原盘DIY，但未选择相关标签 / 种子不是原盘DIY，但是选择了X标签 |
| 「无损」标签但类型非音乐 | [无损标签校验]：无损标签仅供音乐类型使用，请人工确认 |
| 简介图片 <2 张 | [简介图片检测]：简介图片少于两张（当前N张），请人工确认 |
| 简介图片部分不可访问（实测 img 加载） | [简介图片质量]：简介图片部分不可访问，请人工确认（附失败链接清单） |
| 简介中 IMDb/豆瓣链接 >1 个 | [IMDB信息校验]/[豆瓣信息校验]：简介中X链接大于1，请人工审查 |
| 简介既无豆瓣又无 IMDb 链接 | [豆瓣&IMDB校验]：简介缺失豆瓣或IMDB链接信息，请人工确认 |

### 警告级（提示不阻断）

| 判定条件 | 警告消息 |
|---------|---------|
| 官组名不在映射表（疑似国外资源组）且无声明 | [转载来源检查]：疑似国外资源站 X 不需要声明来源 |
| 副标题含"国语"但未选国语标签 | [国语标签检测]：国语标签未选择，副标题带国语，请人工检查是否需要补充国语标签 |
| 动漫不含多集内容（无 SxxExx/第N集/全N集）但带完结/分集标签 | [动漫校验]：动漫不包含多集内容，不应携带X标签，疑似非规范动漫命名，请人工确认 |
| MI 检测到非标准分辨率（宽高不在标准表内） | [分辨率校验]：媒体信息中检测到非标准分辨率 (来自X)，请人工确认分辨率 |
| 选了音频格式但无 MI/DiscInfo 无法校验 | [音频格式校验]：缺少MediaInfo/DiscInfo信息，无法进行音频格式校验，请人工确认 |
| 简介◎片名/◎译名 与副标题分词无交集（中文≥2字词+英文≥3字母，双向去停用词表） | [标题一致性]：简介/副标题内容不一致，请人工确认 |
| 「纯享版」标签 | [特殊标签提示]：上传者携带纯享版标签，请人工检查 |
| 双方均无中字信号（无标签+MI 无中文轨） | [中字标签校验]：发布者未选择中字标签，X未检测到中文字幕，请检查是否有中文字幕截图 |
| 无任何标签 | 发布者当前没有选择标签，请人工审查注意 |
| 非五类支持类型（非电影/电视剧/纪录片/动漫/综艺） | [类型校验]：非建议审核的内容（X），请自行判断（终止后续校验） |

### 豁免/特殊逻辑

- **ZM 官种保护**：组名（标题末段 `-` 后）以 `zm` 开头（不区分大小写）→ 红字警示"ZM官种不能拒绝，有问题请于下方留言评论"、隐藏一键拒绝按钮；转载来源：无需声明引用；中字标签：免校验（20251111 协议）。
- **驻站保护**：标签含「驻站」→ 红字警示"驻站人员发布资源，请勿审核"、隐藏一键通过+拒绝按钮。
- **转载声明关键词**（简介 fieldset 含任一即视为已声明）：观组作品/感谢原制作者发布/官组出品/感谢原作者发布/官方作品/感谢发布者/感谢原发布者。
- **站点映射表 SITE_SOURCES_MAP（57 站，站名→组名别名）**：
  天枢(dubheweb)、好学(hxweb)、幸运PT(LuckAni/luckdiy)、三月传媒(无别名)、铂金短剧(无别名)、财神(csweb)、回声(hsweb)、柠檬(LeagueWEB/lhd)、蟹黄堡(crab)、青蛙(FROGWeb)、末日(agsvweb)、麒麟(hdkweb/kylin)、肉丝(rousiweb)、熊猫(ailmweb/panda)、U堡(ubits/ubweb)、不可梦(zmweb)、憨憨(hhweb)、车站(carpt)、人人(ADWeb/ADE)、不可比(qhstudio/dbtv)、猫站(pterweb/ptertv)、我堡(ourbits/ourtv/ilovetv)、家园(hdhome/hdhweb)、月月(frds)、馒头(mweb/mteam)、不可说(cmct/CMCTV)、TTG(ttg)、U2(u2)、星陨阁(starfallweb)、天空(hdsky/hdsweb)、彩虹岛(chdbits/chdweb)、猪猪(pigoweb)、朱雀(zwex)、52pt(52pt)、下水道(SewageWeb)、北洋园(tjupt)、吐鲁番(TLF)、铂金家(PTHome/PTHWeb/PTH)、时光(HDT/HDTime)、日日(Sunny)、幸运(LuckWeb/LuckDIY)、自然(NatureWeb)、火鸟(ZWEX)、劳改所(DYZ-WEB/DYZ-Movie/DYZ-TV)、兽(beAst)、学校(BTS/BTSchool)、白兔俱乐部(Hares/HaresWeb/HaresTV)、备胎(BeiTai)、烧包乐园(FFansDIY@至尊宝/FFansWeb/FFansTV)、瓷器(HDChina/HDC/HDCTV)、蝶粉(DiscFan)、爱好多(DIY@iHDBits/iHDBits/iHD)、LongPT(LongWeb)、好多油(HDU/DIY@HDU)
- **自动说谢谢**：进入详情页自动点 #saythanks（已说过/不存在则跳过）。
- **视频格式细化**：BDRip/DVDRip/`x26x+Blu-ray`→Encode；Blu-ray + Writing library x264/x265→Encode；Blu-ray + `ID in the original source medium` 且无 x26x→Remux；Blu-ray+AV1/VP8/VP9→Encode。
- **分辨率细化优先级**：MI Video 宽→标准宽表最近邻（3840/2560/1920/1280/640）> MI/DiscInfo 高（≥4320/2160/1440/1080/720/480）> 标题末位分辨率词。
- **音频格式映射**（MI Format→大类）：TrueHD/Dolby TrueHD→**AC3 大类**；E-AC-3/Dolby Digital Plus→AC3；DTS 全系（HD MA/HD HR/X/ES）→DTS；AAC 全系→AAC；MPEG Audio Layer3→MP3（Layer2→Other）；PCM/LPCM→Other；Vorbis/Opus→OGG。
- **HDR/DV 判定**：HDR10 compatible/SMPTE ST 2086/HDR10→HDR10；Dolby Vision Profile 5（dvhe.05/dvh1.05）→**不算 HDR**；P7（dvhe.07）及其他 DV Profile→HDR；HDR Vivid/CUVA→HDR Vivid；Transfer characteristics PQ + Color primaries BT.2020→HDR Unknown。
- **官组提取**：标题末 10 字符按 `-` 分割取末段。
- **种子标题 = 种子名去末尾" - 组名"段**（无标准分隔时取末个无空格 `-` 段）。
- **文件列表**：优先 viewfilelist.php，回退 torrent_info.php [name] 结构；用于原盘/DIY 判定（有无 .mkv）与国语文件名检测。

### 标题词形规范

- 分辨率 `p` 必须小写（`1080P` 报错）；`4K` 必须改写 `2160p`。
- `HDR` 必须全部大写（独立词匹配，`hdr/Hdr` 报错）。
- 标题空格数 ≥5（保证 `片名 年份 分辨率 来源 编码 ... -组名` 分词完整）。
- 年份（19xx/20xx）必须存在，且位于版本词（PROPER/REPACK/READ.NFO/RERIP）之前。
- 视频编码标准词表（整词匹配）：x264/x265/H.264/H.265/AVC/HEVC/VC-1/AVS2/AVS3/AV1/AVS+（H265/H264/MPEG-2 为非标准表，可匹配但属非常用形）。
- 标题禁中文/全角字符。
