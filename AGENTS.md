...
**遗留**：Tab 3 截图端到端验证（当前主线下一步，编辑器最后一块）；发布页重构（含 tag_config 激活，§56.22，字典完成后启动）；MarkStatus 跨订阅写（§59.17 违反，预存在）；runAnalyze 分级（读 DB 跳过重拉）；integration TestE2E_SeedingFullChain flaky（eng2.Start 竞态，§59.35 P3 记录）。
