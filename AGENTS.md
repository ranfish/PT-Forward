...
**版本**：v0.0.997（已发布——三环境 29/249/243 均已最新）。§59.251 恒定 ID 重构收官（18 处真缺陷+六族终扫归零+三部署路径全通）；249 支线三案闭环（is_local 五处→MI/截图恢复/老域名索引十域恢复/木星六副本 PDMS 全产出）；§59.252 D3 hash 终审+原始名落库（v0.0.997，站方元数据噪声免疫）。→ 待办：①三不变式手工验证（§59.251 交付验收——249 改名/删除/重加各操作一次）②CookieCloud 404 排查（249 配置）③转移链 tr RPC 超时观察（249 日志 transfer failed×N——tr 容器负载或网络）④老挂账按需（标题净化族三/发布配置 diff StandardKeys/§59.208 降级链不对称）。
