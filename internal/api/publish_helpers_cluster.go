...
func (h *PublishTorrentsHandler) buildClusterTargets(ctx context.Context, clientUID uint, savePath, name, selfHash string) []comment.DirectTarget {
	if h.db == nil || clientUID == 0 || savePath == "" || name == "" {
		return nil
	}
	members := publish.GetClusterComments(ctx, h.db, clientUID, savePath, name)
	if len(members) == 0 {
		return nil
	}
...
func clusterKeyOf(db *gorm.DB, infoHash string) (clusterKey, bool) {
	...
	err := db.Model(&model.TorrentSnapshot{}).
		...
		Row().Scan(&ck.clientUID, &ck.savePath, &ck.name)
	if err != nil || ck.name == "" {
		return clusterKey{}, false
	}
	return ck, true
...
func (h *PublishTorrentsHandler) syncClusterReviewedByHashes(ctx context.Context, hashes []string, reviewed bool) {
	...
	for _, ih := range hashes {
		...
		seen[ck] = true
		h.propagateClusterReviewed(ctx, ck.clientUID, ck.savePath, ck.name, ih, reviewed)
	}
}
...
func (h *PublishTorrentsHandler) propagateClusterReviewed(ctx context.Context, clientUID uint, savePath, name, selfHash string, reviewed bool) {
	if h.db == nil || clientUID == 0 || savePath == "" || name == "" {
		return
	}
	var siblingHashes []string
	h.db.WithContext(ctx).Model(&model.TorrentSnapshot{}).
		Where("client_uid = ? AND save_path = ? AND name = ? AND is_hidden = 0 AND hash != ?",
			clientUID, savePath, name, selfHash).
...
// 本就是缓存命中，链接一致）。
func upsertClusterScreenshotCache(db *gorm.DB, logger *zap.Logger, ttlDays int, clientUID uint, savePath, name string, shots []string) {
	if db == nil || logger == nil || ttlDays <= 0 || clientUID == 0 || savePath == "" || name == "" || len(shots) == 0 {
		return
	}
	...
	row := model.ClusterScreenshotCache{
		ClientUID: clientUID, SavePath: savePath, Name: name,
		Screenshots: string(data), UpdatedAt: time.Now(),
	}
	err = db.Clauses(clause.OnConflict{
		Columns:   []clause.Column{{Name: "client_uid"}, {Name: "save_path"}, {Name: "name"}},
...
func (h *PublishTorrentsHandler) lookupScreenshotCache(clientUID uint, savePath, name string) ([]string, bool) {
	if h.db == nil || h.screenshotCacheDays <= 0 {
		return nil, false
	}
	var row model.ClusterScreenshotCache
	if err := h.db.Where("client_uid = ? AND save_path = ? AND name = ?", clientUID, savePath, name).
...
func (h *PublishTorrentsHandler) propagateClusterMetadata(ctx context.Context, clientUID uint, savePath, name, selfHash, siteName string) {
	if h.db == nil || clientUID == 0 || savePath == "" || name == "" {
		return
	}
...
func (h *PublishTorrentsHandler) propagateClusterScreenshots(ctx context.Context, clientUID uint, savePath, name, selfHash, screenshotsJSON string) {
	propagateClusterScreenshotsDB(h.db, h.logger, ctx, clientUID, savePath, name, selfHash, screenshotsJSON)
}
...
func propagateClusterScreenshotsDB(db *gorm.DB, logger *zap.Logger, ctx context.Context, clientUID uint, savePath, name, selfHash, screenshotsJSON string) {
	if db == nil || logger == nil || clientUID == 0 || savePath == "" || name == "" || screenshotsJSON == "" || screenshotsJSON == "[]" {
		return
	}
	var siblingHashes []string
...
// 独立获取行不动（尊重显式数据）。
func propagateClusterMediainfoDB(db *gorm.DB, logger *zap.Logger, ctx context.Context, clientUID uint, savePath, name, selfHash, mi string) {
	if db == nil || logger == nil || clientUID == 0 || savePath == "" || name == "" || mi == "" {
		return
	}
	var siblingHashes []string
	db.WithContext(ctx).Model(&model.TorrentSnapshot{}).
...
func (h *PublishTorrentsHandler) propagateClusterPosters(ctx context.Context, clientUID uint, savePath, name, selfHash string) {
	if h.db == nil || clientUID == 0 || savePath == "" || name == "" {
		return
	}
	var src model.TorrentMetadata
