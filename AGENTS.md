...
**版本**：v0.0.989（已发布）。§59.251 恒定 ID 重构+审核五轮 17 处真缺陷收官（不变式测试锁定/级联删除 16 表/限流失效/IYUU 回显等——docs/31 §59.251 实施记录+五轮方法论）；CI 门禁定案（lint v2.6.2+增量严检/vue-tsc -b 铁律/dict 漂移拦截）；**v0.0.995 已发布**（991-995 五连热修：旧库升级兼容层五层+is_local 五处——29 脚本直部/249 删库全新/243 原地升级三路径全通）→ 待办：249 获取链验证（MI+截图用户报障闭环）+三不变式手工验证（§59.251 交付验收）+CookieCloud 404 排查。
