...
func TestEngine_findCandidates_NilProvider(t *testing.T) {
	...
	candidates := e.findCandidates(context.Background(), sourceTorrent{InfoHash: rec.InfoHash, SiteName: rec.SiteName}, e.preloadSites(context.Background(), nil, nil), nil, 1.0, task, nil, nil, nil, nil, nil, nil)
	if candidates != nil {
...
func TestEngine_findCandidates_WithProvider(t *testing.T) {
	...
	candidates := e.findCandidates(context.Background(), sourceTorrent{InfoHash: "ih1", SiteName: "source_site"}, e.preloadSites(context.Background(), nil, nil), e.preloadFingerprints(context.Background(), []string{"ih1"}), 1.0, task, nil, nil, nil, nil, nil, nil)
	if len(candidates) != 1 {
...
func TestEngine_findCandidates_ExcludedAndDisabled(t *testing.T) {
	...
	rec := model.SeedingTorrentRecord{InfoHash: "ih1", SiteName: "source_site"}
	candidates := e.findCandidates(context.Background(), sourceTorrent{InfoHash: rec.InfoHash, SiteName: rec.SiteName}, e.preloadSites(context.Background(), nil, []string{"excluded"}), nil, 1.0, task, nil, nil, nil, nil, nil, nil)
	if len(candidates) != 0 {
...
func TestEngine_findCandidates_WithTargetSites(t *testing.T) {
	...
	candidates := e.findCandidates(context.Background(), sourceTorrent{InfoHash: "ih_match", SiteName: "source_site"}, e.preloadSites(context.Background(), []string{"site_a"}, nil), e.preloadFingerprints(context.Background(), []string{"ih_match"}), 1.0, task, nil, nil, nil, nil, nil, nil)
	if len(candidates) != 1 {
		t.Fatalf("expected 1 candidate, got %d", len(candidates))
	}
...
func TestEngine_findCandidates_GetSiteInfoError(t *testing.T) {
	...
	rec := model.SeedingTorrentRecord{InfoHash: "ih1", SiteName: "source_site"}
	candidates := e.findCandidates(context.Background(), sourceTorrent{InfoHash: rec.InfoHash, SiteName: rec.SiteName}, e.preloadSites(context.Background(), []string{"bad_site"}, nil), nil, 1.0, task, nil, nil, nil, nil, nil, nil)
	if len(candidates) != 0 {
...
func TestExtractSearchKeyword_ChineseTitleFallback(t *testing.T) {
	tests := []struct {
		...
		{"皮克斯短片合集.1984-2012.中英字幕￡CMCT呆呆", "皮克斯短片合集"}, // §59.204 F2: 区间剥除（探针 8 行⊃2 行，宽度放宽验证层收口）
	}
	for _, tt := range tests {
		t.Run(tt.title, func(t *testing.T) {
			kw := ExtractSearchKeyword(tt.title)
