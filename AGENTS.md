...
**版本**：v0.0.989（已发布）。§59.251 恒定 ID 重构+审核五轮 17 处真缺陷收官（不变式测试锁定/级联删除 16 表/限流失效/IYUU 回显等——docs/31 §59.251 实施记录+五轮方法论）；CI 门禁定案（lint v2.6.2+增量严检/vue-tsc -b 铁律/dict 漂移拦截）；**待发 v0.0.990**（df9a065..f19b850：IYUU 空分支补丁+CI 超时+两 flake 修复——v0.0.989 镜像缺 IYUU 补丁，部署须用 v0.0.990）→ 交付用户删 /data 从零部署+三不变式手工验证（改名连续/删除全消/重加全新）。
