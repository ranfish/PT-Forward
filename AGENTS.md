...
3. **Tab1 20 字段数据源对齐讨论**（#1 片名/#2 译名/③渲染器消费成品文本/#3-#6 已定案（§59.226+附+附二+附三——#6 类型五项：五级链+冲突覆盖/episodes 激活/读库值/类型改类别：撞名解决+genre 列资产单点/发布站词表适配）；#7 起（媒介）继续；表格分区重设计等 20 字段过完一次到位——用户明确：讨论完成前不编码）
