...
**当前主线**：种子配置页数据源完善（六 Tab 逐项闭环）。v0.0.587→v0.0.631 主线完成：操作列（获取/清除/重获，资源级粒度统一）、管线统一（BuildTechProfile 三源合并 + DOM 接入）、声明提取（栈解析器 + MediaInfo 拆分 + 致谢幂等）、标签推断贯通 + label 显示、Tab1 字段补全（声道/bit/中文名/类型中文/规格 Remux 修复）、方案 A 预览（PUT 重组+渲染）、列表交互重构（挂载即查+筛选二级页+批量清除）、L2 搜索四层降级（§59.30）。

**支线完成**：幽灵种子巡检 v2（§59.31，检测处置分离 + 三档调度 + 五轮审计收敛，29/243 生产验证）；Token Registry 三阶段（§59.27）；清 seen 重推（§59.33，两轮审计）；侧栏信息架构重排（§59.32）；后台任务+弹窗进度视图 bug 类全站修复（§59.32）；Remux 规格解析修复（v0.0.629）。

**遗留**：Tab 3 截图 / Tab 4 PTGen 端到端验证；发布页重构（含全站标签字典 tag_config 激活，§56.22 dormant → 主线基础数据夯实后启动）；MarkStatus 跨订阅写（§59.17 违反，预存在）；runAnalyze 分级（读 DB 跳过重拉）。
