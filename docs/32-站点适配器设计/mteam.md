...
| 442 | 有聲書 | AuiBook | 3 |
| 451 | 教育影片 | 教育影片 | 3 |
| 409 | Misc(其他) | Misc(Other) | 4 |
| 427 | 電子書 | E-Book | 1 |

#### 成人区子分类（略）

parent=115(有码): 410(SD), 424(SD), 437(DVDiSo), 431(Blu-Ray)
parent=120(无码): 429(HD), 430(SD), 426(DVDiSo), 432(Blu-Ray), 436(0Day), 440(Gay/HD)
parent=445(IV): 425(影集), 433(图集)
parent=446(H-ACG): 411(游戏), 412(动漫), 413(漫画)
...
| 分组 key | 包含的分类 ID | 用途 |
|----------|--------------|------|
| `movie` | 401, 419, 420, 421, 439 | 电影模式 |
| `tvshow` | 403, 402, 435, 438 | 剧集模式 |
| `music` | 406, 434 | 音乐模式 |
| `adult` | 410, 429, 424, 430, 426, 437, 431, 432, 436, 425, 433, 411, 412, 413, 440 | 成人模式 |
| `waterfall` | 全部 34 个子分类 | 瀑布流模式 |
| `list` | 完整分类对象数组 | 分类管理 |
