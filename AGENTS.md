...
3. **Tab1 20 字段数据源对齐讨论**（#1 片名/#2 译名/③渲染器消费成品文本/#3-#7 已定案（§59.226+附~附四——#6 类型五项+撞名排查 3 文件 5 处；#7 媒介 canonical 纯函数单点化）；#8 片源已定案（附五①三源重构+附六② ST 扩展）；#8 片源已定案（附五①三源重构+附六② ST 扩展）；#9 规格已定案（附七① Encode 派生单点化+附八② UHD Remux canonical+DIY 组名族完整体系——PT4 实证 19 样本/extractGroup 真 bug/臆造教训/三层定案/实施清单五项）；#10 分发方已定案（附九①②标题唯一源+ST 反哺+附十③ 词表完备性：印度平台族 ZEE5/SonyLIV/JioHotstar/SunNXT 补词条）；#11 分辨率已定案（附十一——词形态归一现状即定案+4K/8K 展示层归一 2160p+UHD 属片源零改动+1440p 已覆盖）；#12 视频编码已定案（附十二——MI 铁证×V1.05 媒介形态：Writing library 决定 x264/x265 写法权限，WEB 点分/HDTV 无点/原盘 AVC 规范名分层，撤回标题声明优先）；#13 HDR 已定案（附十三——DoVi 系合规保留+DoVi HDR10 冗余修复/SDR 三层分层：存储 SDR 防错配+展示 SDR+重组不写）；#14 bit 已定案（附十四——MI 优先现状/标题侧归一微修/音频位深防污染：值域收缩 8|10|12/codecStyle 即 #12 既有实现确认）；#15 音频编码已定案（附十五——现状即定案零改动：多音轨最高规格/MI 形态映射/Atmos 分离全对齐）；#16 声道已定案（附十六——MI 优先+selectBestAudioStream 评论轨排除：§59.116 基建复用，音轨数已有扣减/选轨缺口补）；#17 音频技术已定案（附十七——现状即定案：值域/MI 三链/位置全对齐）；#18 音轨数已定案（附十八——现状即定案：计数/评论轨双制扣减/与#16 正交）；#19 版本已定案（附十九——标题唯一源+MAR 补全+合并存储/ReleaseVersion 正交维持）；#20 地区码已定案（附二十——20 字段收官：标题唯一源+重组媒介门非原盘不写）。**Tab1 讨论收官并统一实施完成（§59.226 二十一份定案+§59.228 实施：三源重构/ST 扩展/canonical 单点/DIY 族/中文集数/评论轨排除/印度词表/三分区表格三色——8 包回归全绿）**；表格分区重设计等 20 字段过完一次到位——用户明确：讨论完成前不编码）
