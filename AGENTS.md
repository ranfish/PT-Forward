...
**遗留**：**§59.155 幸运专线切片 1 完成 v0.0.805**：①dict/type.json 8→19 词条（+短剧/MV/演唱会/有声书/无损音乐/游戏/软件/电子书/学习/漫画/成人）——**双词表**（dict→解析+前端 gen-dict；titleparser/data/standard_keys.json→DB 播种 syncStandardKeys，漏后者则 DB 停 8 条）；②sites.publish_form_config 列+PublishFormConfig struct（enabled 位/form_fields 11 逻辑域常量/value_mappings label→standard_key 化含 Auto *bool 条件标签/pre_audit_url）；③migration 27 幸运落库（77 条映射化入：type 10/10、tags 13/17 英语/首发/原创/禁转 auto:false、codec 双键 vc 双形态；留空=待标注切片 3 HTML diff 兜底）+ migration 28 回归审核补 tag_config_legacy.mode=checkbox_span。**部署坑：build-backend 二次构建 restart 竞态——部署后必须数据级验证**（standard_keys 计数/migration 记录，不能只看脚本退出码）。**§59.156 切片 2 完成 v0.0.806-807**：executor.go PublishExecutor 十步流程（ResourceResolver 簇数据→form_config Enabled 门→ExportTorrent 本地导出零拉取→域值组装 LookupStandardKey 归一+ValueMappings 反查→tags 判据引擎+auto:false 过滤+overrides 人工通道→renderDescription→pre-audit 官方预检（mode 从字段名后缀提取）→dedup+UploadTorrent→pusher 加种→ResultRecord 落库）；TagApplier SpanField 参数化（tags[4][]）；灰度机制退役（tagConfigFromFlag/tagModeBySite/SetTagApplierSites/KeyTagApplierSites 全删）；API POST /publish/seeds/execute（dry_run 模式）；**243 爱情抓马 DryRun 端到端=§59.150 官方 100 分验证完全一致**（含 audiocodec 11 组合键/medium 7 Encode 碟转压/英语 auto:false 排除）。回归三修：standard_keys 域名 video_codec/audio_codec、migration 29 音频组合键、tags 双形态键+Tags 构造遗漏。**二次回归审核 v0.0.809**：pre-audit 走站点代理配置+发布流程脱离 HTTP 生命周期（Background+10min，§59.51 教训）。**补丁系列（信封对齐/接线/三语言占位 v0.0.814-2）**：form-config 三端点 Success 信封对齐+SiteDetail import 补线+占位过滤 value=0 语言无关化（三语言实测 label 硬编码——语言映射伪需求）。**v0.0.810 站点代理解析公共单点**：site.ResolveSiteProxy(db,ctx,site)（proxy_url 优先→use_global_proxy 读 httpProxy→空=直连）——发布链站点维度 HTTP 统一单点（adapter/预检），禁裸 client。**回归审核三轮修复 v0.0.808**：TagArrayFields 多值通道（checkbox 数组同名字段重复）+DryRun 检查点后移（渲染+预检后）+截图 [img] 前置（官方验证格式 ≤8 张）——**243 DryRun pre-audit 100 分零错误零警告终态**（=§59.150 官方验证）。**§59.157 切片 3 完成 v0.0.811**：配置中心 UI——POST form-config/{get,parse,apply} 三端点+ParsePublishFormHTML（goquery 六族识别+tags checkbox 三形态+占位过滤）+MergeDraftWithCurrent（语义继承+diff 三分类 matched/changed/added/removed）+FormConfigPanel 组件嵌入站点详情页——**仅 is_target 站渲染**（发布语义=目标站；幸运源+目标双角色命中；用户两轮纠正定案）、侧栏+独立路由双撤（消灭第二入口）；HTML 即弃 L2。29 幸运真实 HTML 48KB 端到端：11 域全识别 diff=0（切片 0 审计机器复现）+apply 审计落库。**§59.158 切片 3.5 完成 v0.0.815**：targets 端点+PublishExecuteModal 两段式（DryRun 预检明细+passed 门控→确认提交上传加种）+PublishTorrents 选站发布按钮激活；execute 信封对齐（裸写教训二连——含错误响应 Error 统一）+条件标签 options 接线（auto:false 人工通道 UI）。**§59.159 重复上传识别 v0.0.817（切片 4 首发实战发现）**：NP 重复上传 302 existed=1（pt-nexus 日志佐证）被默认跟随落点详情页误判新发布——uploadGeneric CheckRedirect 捕获+stderr 文本 fallback（PTNexus 竞品调研同款）→ PublishResponse.IsExisting/ExistingID 首次投产 → executor uploaded_existing/pushed_existing 分流（加种照做=辅种语义）；dedup 前置对非 pieces API 站静默跳过定案（标题搜索兜底误判风险大于收益不做）。**切片 4 实战 19-commit 破案链收官 v0.0.818**：七层洋葱全破（dedup 静默/词边界/双副本/tags 断头/takeupload 端点/资产投递/LuckAudit 审核）+**发布二元化四轮定案**（仅成功页种子：302 finalID+uploaded=1；body 提取全删防误推——53342/52394 推荐位误推根治；已存在=失败[辅种范畴]）+完整投递（quote=Tab2 声明置顶/截图无引号后置/pt_gen/dburl/uplver/匿名站点默认）+NP 转发改种 hash 变三例实证。**五轮修复+补推 UI v0.0.818-819**：CheckExists 源 hash 快路径废除（源在≠新种在——53511-16 六种误判未推根因，用户实战发现）+PushOnly 补推通道（push_only+torrent_id，记录回放架构 downloader_id+save_path 落库）+发布日志页补推按钮（成功回写 Status/Seeded）+NP 族下载 URL 双副本收敛（BuildNexusDownloadURL——§59.26 同型第三例）。**实战首两例 53534/53535 pushed 成功（新架构投产验证）**。回归审核补丁 v0.0.821：duplicate/existing 终态补落库（recordResultFull 单点）+多段修改脚本 assert 中断丢写入教训（分文件验证调用点）。**下一步：批次多站验证+LuckAudit 过审观察+错加无关种清理**。**§59.153 scoring_logs 同思路治理+29 库重建式治理方法论（v0.0.803）**：seeding_torrent_records 加 last_score/last_cycle_id 列 UPSERT + rule_evaluator preload 改读列（293 万行 MAX(id) 子查询 1.8s→毫秒点查）+ scoringCutoffHours 72h→24h（243 自动清至 94.5 万行）+ **db_vacuum 月度 cron**（`0 5 1 * *`，删除后空闲页不归还 OS——243 实测 3400 万行删后仍 8.9G 手动 VACUUM→1.5G 的教训）；29 库治理**重建式>删除式**方法论（ATTACH+INSERT SELECT 只拷保留的 5%，rowid 二分定位表尾，16G→662M/52s vs 删除式一上午未完；三坑：dbstat 全库扫描/跨库需 ATTACH/各表时间列名不同；详见 docs/31 §59.153 补充）。**§59.154 dvhe.09 判据破案+防再犯双保险（v0.0.804）**：三路排查（243 语料 4352 条三态分布+95 站+UBits 全站搜索零命中+Dolby Profiles v1.5 官方表定谳）——**P9=8-bit AVC+SDR 兼容层非 HDR10**，tag_inferer.go dvhe.09 摘出双勾组归"仅 DV"（dvhe.08 独占双勾）；dvhe.05 首实证（UBits SNYLV dvhe.05.06 BL+RPU 无 ST2086）单测三态锚定；v0.0.804 事故教训：`*.db` 不匹配 `.db.old16g` 后缀致 16GB 备份被 add -A 带入 commit/push 卡死（远端未污染）——防再犯=`.gitignore` 收敛 `*.db*` 通配 + `.githooks/pre-commit`（core.hooksPath）拦 .db 族+>50MB 大文件双保险；远端下载器（12 禁操作环境）绕行=回源站抓详情页 MI（curl -x 代理+sites cookie）。**§59.152 数据治理融合方案已实施 v0.0.802**（Vertex 三层：UPSERT 最新值 current_uploaded + site_traffic_flows 写时聚合 + traffic 明细 30→1 天 + WAL 每 6h checkpoint——243 稳态验证 1.5GB/10min 聚合正常，首轮伪增量已清）。§59.131-134 完成；§59.148 旧发布地基拆除 v0.0.794；**§59.149-151 幸运专线切片0+数据源修正六轮推演 166/166 全通过 v0.0.795-800（附2 数据准确性 48/48 假纯英；附3-7 破案链收官：Zootopia 两簇/vc 双形态语义/数据层三性/P5-P7 探针（HDR10·10+互斥规则）/英语条件映射 lucky_english_audio（Buried.Alive 实证必须勾）；判据单测五组锚定；推演资产入库 tools/luckpt-audit）**——pre-audit API 官方预检闭环/MI 唯一真相判据八条（HDR Profile 语义/语言双语匹配/粤语复合/媒介 title 词优先）/recompute-profiles 自愈端点/站点适配问询铁律——链A/B+组链运维层-9100 行：fetchSourceInfo 零拉取违规/OnTorrents 自动入队违规/tracker 页等随葬；复用组件保留待切片2新执行器；LifecycleManager 待拆项）。**站点发布配置中心方案已定案待实施**（§59.146-147：R3-5 TagApplier 接线+幸运专线切片0-4+3C6L九点定案+HTML 上传半自动+分类三层结构）。**当前打磨线**：§59.135 技术规格表展示同件（SeedTechDescriptions 组件）→§59.136 Tab1 视觉同步+util.StripSiteOperationMarkers 运营标记公共方法 →§59.137 朋友站源截图特化（采集层写侧丢弃+settings 排除表，243 重取验证 source=0→mpv 全量）→§59.138-143 /publish/torrents 打磨线**全部收官**：1 删发布就绪列/2 sites tooltip/3 三域搜索 CJK 门控（29 冷缓存教训）/4 类型列/5 前端排序/6 ready 行操作分化（预览种子 initialPreview 直开+四字文案对齐）/7 撤回（keep-alive 误判）/8 绿底保持现状/+ready 值域 bug 修复（前端 ready/pending vs 后端 true/false 不匹配三态同数据）/+禁转簇发布页隐藏（exclude_forbidden，776-772=4 分账核对，配置页维持可见）+分类体系定案（保持 8 类：type=选站过滤层/细分归 tag 域/R3-5 表单映射；剧集改名 dict canonical+gen-dict；categoryDisplay 8 色公共单点）。**§59.144 QB0 残留簇三层根因**：syncSnapshots 空列表早退删除（下载器清空=全部消失应标 hidden——8-26 QB 清空后快照永冻 is_hidden=0 混入 ready 计数）+GetAllTorrents Debug→Warn+手动清 4 行；下载器/路径筛选天然隔离定案（簇键三元组成员即筛选域）。**下一步：R3-5 TagApplier 灰度接线+R3-6 team 域**。**R3-5 TagApplier 接线+R3-6 team 域待做**。——术语定名：一种多站=簇中心扇出（PublishTorrents 簇口径 v0.0.777 已完成：ready 过滤+副本数+站点列表+deep-link 引导+覆盖/向导/detect-source 现场链路全移除）；一站多种=站中心灌入（PublishData Tab 化 v0.0.778 已完成：目标站选择+ready 簇+已存在禁选+批量占位，CrossSeedPanel 维护模式消灭老向导第二入口）；老向导拆除（④ v0.0.780 已完成：CrossSeedPanel 只剩维护形态 -621 行/三向导组件删除/死 API 与类型清理/PublishList 死挂载移除——§59.134；后端旧端点待 R3-5 后清）；TagApplier 灰度接线+team 域（R3-5/6 待做——下一步）。Round 3 六问定案见 §59.124-130。**遗留清理六项全部完结（2026-08-27）**：①批量截图并发限流（§59.58 观察）②MarkStatus 订阅隔离（§59.120，v0.0.773）③runAnalyze 改判随发布页重构废弃（§59.121 发布零拉取定案）④E2E flaky 根治（§59.122 SQLite cache=shared，v0.0.774，顺带评分清理阈值配置化）⑤海报死链重触发（维持随发布页重构——当前数据零受影响+重取自愈，§59.119 观察项说明已补）⑥站点别名域名随版本同步（§59.123 migration 24，v0.0.775，243 实测 10/10）。
