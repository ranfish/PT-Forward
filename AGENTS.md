...
3. **Tab1 20 字段数据源对齐讨论**（#1 片名/#2 译名/③渲染器消费成品文本/#3-#7 已定案（§59.226+附~附四——#6 类型五项+撞名排查 3 文件 5 处；#7 媒介 canonical 纯函数单点化）；#8 片源已定案（附五①三源重构+附六② ST 扩展）；#9 规格定案①（附七——Encode 派生单点化：后端生效规格+前端纯展示+列表页 MI 铁证丢失修复），② 词汇完备性讨论中；表格分区重设计等 20 字段过完一次到位——用户明确：讨论完成前不编码）
