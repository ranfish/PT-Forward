...
**版本**：v0.0.1029（29 已部署）。近期：§59.260-268 声明幂等/4K scope/凭证拦截三防线/HDR Vivid canonical/codec 折叠；§59.269-272 修道院标签三修+存量工具（cmd/xdy-codec-fix 独立小工具 593 全修）；§59.273-274 幸运四修+审核门+相邻槽终态护栏；§59.275 batch-review 运维内部接口+簇口径约定；§59.276 一站多种页面级发布+完成自动刷新；§59.277-278 幸运英语标签站规（auto:false×零键双卡断）+中字关键词族终版（中字/中英字幕/硬字幕/特效字幕/简繁族——产地忽略+无中字剥除）+HDR 标签标题对齐；§59.279 pt29 批量获取卡死双层根治（pprof 诊断基建 debug.pprof→localhost:6060+AnalyzeLocalArtifacts 限时 90s+WAL 310MB 救援）；§59.280 OTA 更新代理开关（otaUseProxy 总开关——fnos 直连挂起案）。全案详见 docs/31 §59.260-280。挂账观察：①修道院 320 无标签存量（待跨环境元数据补齐）②幸运三边界缺口（按需）③Beverly DoVi 截图（源文件非标准不修）④WAL 定期维护任务（按需）。下一步主线：源站<-->发布站逐站适配（交叉进行）。
