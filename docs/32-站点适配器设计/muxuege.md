...
| 站点框架 | NexusPHP |
| 特殊规则 | 游戏类资源需候选审批；种子自动随机促销（20%概率）；>20GB 自动免费；发布满 1 个月自动永久 2x 上传；48 个制作组（含大量公网组）；编码含 HDR 10/TXT/PDF；分辨率含 540p；31 标签 |
| 发布页面 | `upload.php` |
...
- **任何人都能发布资源**
- 有些用户需先在**候选区**提交候选
- **游戏类资源**：仅上传员及以上等级，或管理组指定用户可自由上传；其他用户须候选

### 0.5 允许的资源

| 类型 | 说明 |
|------|------|
| HD 视频 | 蓝光原碟/HD DVD/remux/HDTV/高清重编码(≥720p)/高清DV |
| SD 视频 | 高清来源标清重编码(≥480p)/DVDR/DVDISO/DVDRip |
| 无损音轨 | FLAC/APE 等 + CUE 表单 |
| 多声道音轨 | ≥5.1 声道 (DTS/DTSCD 等) / 评论音轨 |
| PC 游戏 | 必须为原版光盘镜像 |
| 预告片 | 7 日内发布的高清预告片 |
| 软件/文档 | 与高清相关 |
...
| SD upscale | 标清 upscale 或部分 upscale |
| 低质量 SD | CAM/TC/TS/SCR/DVDSCR/R5/HalfCD 等 |
| RealVideo | RMVB/RM/flv |
| 单独样片 | 样片须和正片一起上传 |
| 有损音频 | <5.1 声道的有损 MP3/WMA 等（2.0+ 国语/粤语例外） |
| 游戏相关 | 硬盘版/高压版/非官方镜像/第三方 mod |
| 压缩文件 | RAR 等 |
| 重复资源 | Dupe 判定见下 |
| 敏感内容 | 色情、敏感政治话题等 |
| 损坏/垃圾文件 | 病毒/木马/广告等 |
...
- 同视频高优先级版本使低优先级为重复
- 同视频高清版本使标清为重复
- **动漫特例**：HDTV 和 DVD 版本有相同优先级
- 同来源同分辨率重编码：按发布组确定优先级
- **总会保留**一个 DVD5 大小（~4.38GB）的最佳重编码版本
- 不同区域/不同配音字幕的 Blu-ray/HD DVD 原盘**不视为重复**
- **每个无损音轨只保留一个版本**（分轨 FLAC 优先级最高）

**断种豁免**：连续断种 **45 日以上**或已发布 **18 个月以上** → 不受 Dupe 约束

### 0.8 打包规则（试行）

允许打包：按套装售卖的电影合集 / 整季电视剧综艺动漫 / 同一专题纪录片 / 7 日内预告片 / 同一艺术家 MV 和 5 张以上专辑 / 发布组打包

打包要求：同来源媒介/同分辨率/同编码。电影合集发布组也必须统一。
...
- 电影/电视剧/动漫：**必须**包含海报/封面；建议包含截图、文件详情、演职员和剧情概要
- 音乐：**必须**包含专辑封面和曲目列表
- PC 游戏：**必须**包含海报/封面
- NFO 图写入 NFO 文件，**不要粘贴到简介里**
- 电影和电视剧**必须**包含外部信息链接（如 IMDB）
...
| 规则 | 慕雪阁情况 | PT-Forward 处理 |
|------|-----------|-----------------|
| §30.5 禁止 9KG/成人 | 明确禁止色情/敏感内容 | 源站带成人标签→不转发 |
| §30.5 禁止禁转/独占 | 未特别强调但仍适用 | 源站带禁转标签→不转发 |
| §30.5 CatEDU 默认禁转 | 无特别规定 | 按全局规则处理 |
| 标题规范 | 0DAY 格式 | 转发时按规范重写标题 |
| IMDB 链接 | 电影/电视剧必须 | 从源站提取 IMDB 链接填入 url 字段 |
| 海报/简介 | 必须 | 从源站提取 |
| Dupe 检查 | 来源优先级 + 断种豁免 | 发布前搜索站内 |
| 游戏资源 | 需上传员等级 | 确认账户权限后再转发 |
| 无音频编码 | 表单无此字段 | 不填写 |
| 无匿名发布 | 表单无此选项 | 不勾选 |
| >20GB 自动免费 | 大体积种子自动免费 | 了解即可 |

---

*文档维护：PT-Forward 开发团队*
*最后更新：2026-04-23*
*数据来源：rules.php + upload.php 表单采集*
