...
**版本**：v0.0.863（已发布——release-tag 脚本自动同步）。

**主线状态一句话**：一种多站收官（§59.149-165）；**一站多种已实施+端到端验证**（§59.166：execute-site-batch 任务化/串行+站点间隔/轮询进度/断线恢复 + 前端种子中心终稿（下载器/路径记忆→三态筛选 全部/可发布/已发布·目标站四终态口径→多选→选站弹窗单选右下发布/SiteTiles 公共组件/已覆盖不阻塞 dedup 兜底）——Suspiria tid=1858 实战）。**两主线闭环（§59.165/166）**；**当前主线：憨憨源站适配**（§59.167：仅源站角色/RSS 采集验证运行中不绑下载器/禁转徽章采集/去重修复——用户授权适配期专用 RSS）。
...
- **站点下线=删除记录（全站通用铁律）**：下线站点 migration 应 DELETE sites 记录（全系统不可见才是"下线/取消支持"语义；supported_sites blocked 拦新增保留为第二道防线）（2026-09-04 用户定案）
