...
**当前主线**：§56.33 手动转发字段采集链路重构已完成并验证（14/14 副标题获取成功）。§56.34 种子技术特征模型**全部完成**（步骤 1-5，TechProfile 体系已接入运行时 + 旧体系已废弃）。§56.35 标题重组范式引擎**阶段 1 已完成**（codecStyle + v1.05 完整 order + func_overrides），阶段 2-3 待做。
...
**待办**：
- §56.35 阶段 2：3 种范式（dot/space/chinese_prefixed）+ 自动推断 + 8 站配置
- §56.35 阶段 3：音乐类 + 特殊站（U2/TTG/HDRoute）+ 分类切换 + 片名保护
- 测 /submit 真发布（高优先级）
