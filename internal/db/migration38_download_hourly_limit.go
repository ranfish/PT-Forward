...
// 次/小时，0=全局默认 95）。用户定案：优堡等实测无此严限制的站点可在
// 站点管理-详情-网络 调高；孤儿恢复批量下载曾被全局 95/hour 误拦（PT30 实证）。
func addDownloadHourlyLimit(gormDB *gorm.DB) error {
	...
	return gormDB.Exec("ALTER TABLE sites ADD COLUMN download_hourly_limit INTEGER NOT NULL DEFAULT 0").Error
}
