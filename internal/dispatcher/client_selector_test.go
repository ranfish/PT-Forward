...
func TestClientSelector_LeastLoad(t *testing.T) {
	provider := &mocks.DownloaderProvider{
		GetFn: func(id string) (model.DownloaderClient, error) {
			clients := map[string]*mocks.DownloaderClient{
				"a": {Name: "a", GetSeedingTorrentsFn: func(ctx context.Context) ([]*model.TorrentInfo, error) { return make([]*model.TorrentInfo, 50), nil }},
				"b": {Name: "b", GetSeedingTorrentsFn: func(ctx context.Context) ([]*model.TorrentInfo, error) { return make([]*model.TorrentInfo, 5), nil }},
			}
			c, ok := clients[id]
			if !ok {
				return nil, &model.AppError{Code: 40400, Message: "not found"}
	...
	if selected != "b" {
		t.Errorf("least_load should select client with fewest torrents, got %s", selected)
	}
}
