...
**版本**：v0.0.1027（29 已部署）。近期：§59.260-268 声明幂等/4K scope/凭证拦截三防线（interceptor.go Generic+NexusPHP 双接入）/HDR Vivid canonical/codec 折叠（FoldStandardKey 词条数据化）；§59.269-272 修道院标签三修+存量工具（cmd/xdy-codec-fix 独立小工具：codec+tags 双补/tag-only/593 全修收官）；§59.273-274 幸运四修+审核门（Other 三域 Label 兜底/中字启发/ST×SPEC 去重+相邻槽终态护栏/未审核禁发布+选种门）；§59.275 batch-review 收归运维内部接口+簇口径约定；§59.276 一站多种页面级发布+完成自动刷新；§59.277 幸运英语标签站规（auto:false×零键双卡断——Label 直配例外路径）。全案详见 docs/31 §59.260-277。挂账观察：①修道院 320 无标签存量（待跨环境元数据补齐→tags-map 标题联接→tag-only 批补）②幸运三边界缺口（Other 键/3D 折叠/DTS-HD HR——按需）③PT30 Beverly DoVi 原盘截图花屏（源文件非标准——手动截图，已诊断落盘不修）。下一步主线：源站<-->发布站逐站适配（交叉进行）。
