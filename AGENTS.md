...
**版本**：v0.0.997（已发布——三环境 29/249/243 均已最新）。§59.251 恒定 ID 重构收官（18 处真缺陷+六族终扫归零+三部署路径全通）；249 支线三案闭环（is_local 五处→MI/截图恢复/老域名索引十域恢复/木星六副本 PDMS 全产出）；§59.252 D3 hash 终审+原始名落库（v0.0.997，站方元数据噪声免疫）。→ **§59.251 主线已收官**（三不变式 249 生产 API 验证全过——改名连续/删除全消/重加全新，专用测试下载器零损伤；CookieCloud 404 销案）。待办：①转移链 tr RPC 超时观察（249 日志——tr 容器负载或网络）②老挂账按需（发布配置 diff StandardKeys）③§59.252/253/254 实战观察（hash 终审/标题净化四族/PTGen 第四源+英文名兜底链）④§59.208 降级链不对称（维持挂账——案例线/重构线双触发）。§59.254 发布前置五项已实施（v0.0.998+四提交——genre 资产/英文名兜底链/第四源合并/D 拒发+预览警示，29 已部署）。下一步主线：源站<-->发布站逐站适配（交叉进行）。~~标题净化族三~~（09-20 销账——§59.253 族三四实施：裸时间尾+组名锚形态免疫收官，v0.0.998）。
