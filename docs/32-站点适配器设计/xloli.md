...
**重要约束**：
- **禁止下载 9KG 版块资源**
- **禁止发布资源到特别区（9KG）**：适配器只能使用综合区（browsecat，data-mode=4）的分类
...
#### 9KG 区（`specialcat`，data-mode='6'）— 11 个（禁止使用）
...
**注意**：9KG 区分类极其丰富（11 个成人内容分类），但适配器 **必须禁止使用**。
...
适配器 **必须硬编码限制**：
- 只使用 `browsecat`（data-mode=4）的分类
- 禁止使用 `specialcat`（data-mode=6）的任何分类（418-430）
- 禁止下载 9KG 区资源
