...
func (rt *Router) RegisterWithEndpointLimits(mux *http.ServeMux, corsOrigins []string, rateLimitEnabled bool, rateLimitGlobal, rateLimitWrite, rateLimitDownload int) {
	...
	dlHandler := rt.corsMW(downloadLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.clientHandler.ServeHTTP)))))
	...
	filterHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.filterHandler.ServeHTTP)))))
	mux.Handle("/api/v1/filters/rules", filterHandler)
	mux.Handle("/api/v1/filters/rules/", filterHandler)

	notifyHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.notifyHandler.ServeHTTP)))))
	mux.Handle("/api/v1/notifications/channels", notifyHandler)
	mux.Handle("/api/v1/notifications/channels/", notifyHandler)

	settingsHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.settingsHandler.ServeHTTP)))))
	mux.Handle("/api/v1/settings", settingsHandler)
	mux.Handle("/api/v1/settings/", settingsHandler)

	seedingHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.seedingHandler.ServeHTTP)))))
	...
	deleteRuleHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.deleteRuleHandler.ServeHTTP)))))
	mux.Handle("/api/v1/seeding/delete-rules", deleteRuleHandler)
	mux.Handle("/api/v1/seeding/delete-rules/", deleteRuleHandler)

	reseedHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.reseedHandler.ServeHTTP)))))
	mux.Handle("/api/v1/reseed/tasks", reseedHandler)
	mux.Handle("/api/v1/reseed/tasks/", reseedHandler)

	publishHandler := rt.corsMW(writeLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.publishHandler.ServeHTTP)))))
	...
	dashboardHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.dashboardHandler.ServeHTTP)))))
	...
	systemHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.systemHandler.ServeHTTP)))))
	...
	torrentEventHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.dashboardHandler.ServeHTTP)))))
	mux.Handle("/api/v1/torrent-events", torrentEventHandler)
	mux.Handle("/api/v1/torrent-events/", torrentEventHandler)

	iyuuHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.iyuuHandler.ServeHTTP)))))
	...
	fingerprintHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.fingerprintHandler.ServeHTTP)))))
	mux.Handle("/api/v1/fingerprints", fingerprintHandler)
	mux.Handle("/api/v1/fingerprints/", fingerprintHandler)

	trackerHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.trackerHandler.ServeHTTP)))))
	mux.Handle("/api/v1/tracker/members", trackerHandler)
	mux.Handle("/api/v1/tracker/members/", trackerHandler)
	mux.Handle("/api/v1/tracker/history", trackerHandler)
	mux.Handle("/api/v1/tracker/history/", trackerHandler)

	lifecycleHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.lifecycleHandler.ServeHTTP)))))
	mux.Handle("/api/v1/lifecycle/config", lifecycleHandler)
	mux.Handle("/api/v1/lifecycle/config/", lifecycleHandler)
	mux.Handle("/api/v1/lifecycle/backpressure", lifecycleHandler)
	mux.Handle("/api/v1/lifecycle/backpressure/", lifecycleHandler)

	cookiecloudHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.cookiecloudHandler.ServeHTTP)))))
	...
	ptgenHandler := rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(http.HandlerFunc(rt.ptgenHandler.ServeHTTP)))))
...
func (rt *Router) public(fn http.HandlerFunc) http.HandlerFunc {
	chain := rt.corsMW(rt.secMW(fn))
	return chain.ServeHTTP
}

func (rt *Router) protected(fn http.HandlerFunc) http.Handler {
	return rt.corsMW(rt.rateLimitMW(rt.authMW(rt.secMW(fn))))
}
