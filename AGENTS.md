...
**遗留**：发布页重构（含 tag_config 激活，§56.22，下一步主线）；批量截图并发限流（>20 路挤爆代理，§59.57 观察）；MarkStatus 跨订阅写（§59.17 违反，预存在）；runAnalyze 分级（读 DB 跳过重拉）；integration TestE2E_SeedingFullChain flaky（eng2.Start 竞态，§59.35 P3 记录）。
