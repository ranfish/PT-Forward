...
2. PT31/243 OTA v0.0.881（§59.170 F2 复验 + §59.171 附生效；PT31 批量重获治愈 ~223 簇存量）
...
7. 支线：OTA 更新验证待用户执行（§59.167 三合一：migration 33 站点同步+migration 4 无限重启修复+Docker 日志治理（默认 error 级/密码直打印））
...
**活跃观察项（未闭环）**：
- §59.194 挂账：降级轮（mixed/purify/B 补线）无 rc 日志可观测盲区（午夜凶铃 Res 假冲突根因已修 v0.0.924，净化词侧待重试观察）
- §59.196 数据修复待办：PT30 批量获取时段错挂受害面扫描（纯中文名形态）→删错误行→重获（顺序铁律：先修后清）；PT30 15:5x 失联待恢复
