...
去重后实际为 **23 个**（rarbg/mp4ba/fgt/hao4k 在数组中重复出现）：
...
> 以下内容来源于 2026-04-23 采集的 Wiki 页面（需 JWT Cookie 认证，3 个页面均 200 成功）。

### 6.1 上传规则（Wiki）

#### 6.1.1 上传总则

- 做种时间不足 **48 小时**（非标准 24 小时），或故意低速上传 → 警告、强制候选甚至取消上传权限
- 发布者获得 **双倍上传量**
- **转载禁止删除原站后缀、修改文件名、修改文件夹结构**

#### 6.1.2 不允许的资源（补充）

除了标准 NexusPHP 禁止项外，麒麟 Wiki 明确禁止：
- **别的 PT 站注明禁转的资源，或于限转期间转发**

#### 6.1.3 Dupe 共存原则

HDKylin 的 dupe 规则有 **共存原则**（与标准 NexusPHP 重要差异）：
- **不同小组作品允许共存**
- **不同分辨率作品允许共存**
- **不同编码作品允许共存**
...
- 动漫 HDTV/DVD 同优先级
- 断种 **45 天** 以上或发布 **18 个月** 以上 → 不受 dupe 约束
- 来自 BT/网盘资源（试行）：站内无 → 保留；站内有更优版本 → dupe

### 6.2 盒子规则（Wiki）

| 项目 | 规则 |
|------|------|
| 盒子定义 | 上下行带宽较大的服务器/VPS/主机 |
| 单种上传限速 | **100 MB/s（800 Mbps）** |
| 做种上传限制 | 72 小时内只享受 **种子体积 ×3** 的上传量 |
| 登记方式 | 系统自动识别 + 控制面板手动登记 |
| 阻断盒子商 | **Digital Ocean、Linode、Vultr**（2024 年 7 月起） |
| 未登记处罚 | 一次解封机会 + 警告 4 周，第二次不予解封 |
| 超速处罚 | 一次解封机会 + 警告 8 周，第二次不予解封 |
| 免限 | VIP 用户组、工作组、管理组 |
| 反作弊 | 秒删刷流 → 严厉处罚；虚假信息登记 → 作弊处理；VPS 反代洗白 → 直接禁用账号 |

### 6.3 H&R 规则（Wiki）

| 项目 | 规则 |
|------|------|
| 达标条件 | 从下载开始累计做种 **48 小时** 或 上传量 > **2 倍种子体积** |
| 考核期 | 下载完成后 **240 小时**（10 天）内完成达标 |
| 免疫 | 工作组 + VIP |
| 初始 HP | **10 点**，每个违规扣 1 点 |
| 封禁线 | 累计超过 **10 个**未达标 H&R → 自动 ban |
| H&R 数字含义 | 考核中 / 未达标 / HP 数量 |
...
| 每种最多认领 | **20 人** |
| 每用户最多认领 | **1000 个** |
| 不达标扣费 | 删除并扣除 **1000 魔力**（非认领首月） |
| 主动放弃扣费 | **500 魔力** |
| 达标奖励 | 正常魔力值的 **2 倍**（每月月初结算） |
| 达标标准 | 每月做种 ≥360 小时 **或** 上传量 ≥ 体积 **3 倍** |
...
| 项目 | 页面表单 | 审种脚本(v1.3.15) | 差异 |
|------|---------|------------------|------|
| 音频编码 | 19 个（含 Opus/DTS-HD HR/MPEG） | 17 个（**无 Opus/DTS-HD HR/MPEG**） | 脚本滞后 |
| 分辨率 | 7 个（含 2K/1440p） | 6 个（**无 2K/1440p**） | 脚本滞后 |
| 制作组 | 9 个（含 Kylin/StarfallWeb/RedLeaves） | 8 个（**无 Kylin/StarfallWeb/RedLeaves**） | 脚本滞后 |
| 地区 | 18 个 | 0 个（area_constant 为空） | 脚本未定义 |
| 分类 | 14 个页面 + 脚本有 Comic/Picture | 脚本多 416(漫画)/418(图片) | 页面未展示 |

### 7.2 脚本已知 BUG

1. **AV1 编码 BUG**：标题含 AV1 时 title_encode=12（不在 encode_constant 中），与用户选择 AV1(7) 不匹配会报错
2. **黑名单检测已注释**：代码保留但第 905-908 行被注释掉，当前不执行
3. **国语/中字/英字标签检测已注释**：代码保留但第 973-986 行被注释掉

### 7.3 Wiki 规则未覆盖的脚本逻辑

| 脚本规则 | Wiki 规则中是否提及 | 说明 |
|---------|-------------------|------|
| 标题中文检测 | 未提及 | 脚本特有，Wiki 仅说"要求规范填写" |
| 副标题必填 | 未提及 | 脚本特有 |
| 简介 MediaInfo 必填 | 未提及 | 脚本特有 |
| 图片 ≥2 张 | 未提及 | 脚本特有 |
| 官种/驻站标签体系 | 未提及 | 脚本特有 |
| 黑名单制作组 | 未提及 | Wiki 无，脚本保留但已注释 |
| 大包标签(>1TB) | 未提及 | 脚本特有 |

### 7.4 Wiki 规则未覆盖的适配器影响

| Wiki/脚本规则 | 对适配器的影响 |
|-------------|---------------|
| 做种 48 小时 | 转发后需保持做种 ≥48h |
| 转载禁止修改文件名/后缀/文件夹 | 适配器不能修改 torrent 内容 |
| Dupe 共存原则 | 同片不同组/分辨率/编码可同时转发 |
| 禁止转载禁转资源 | §30.5 已对齐 |
| 盒子 72h 内体积 ×3 | 如使用盒子需注意上传量限制 |
| H&R 48h/240h | 转发下载的资源需注意 H&R 达标 |
| 认领系统 | 1 天后可认领，达标标准 360h 或 3 倍体积 |
...
> 来源：用户提供站方种审脚本全文分析（HDKylin-Torrent-Assistant v1.3.15，/tmp/0 归档）。与 docs/38 共性矩阵的站特例部分；共性规则（C1-C12）标注于判定条件内。
> 三站（AGSV/CS/HDK）中**最宽松**：未信任组黑名单/BBCode/分集/字幕标签联动等 AGSV 系规则全部停用，且**无错误时自动点击一键通过（自动过审）**。
> ⚠️ 脚本存在系统性大小写缺陷：多处用大写词（"HDK"/"HDKMV"/"WEB-DL"/"REMUX"/"HDTV"）去匹配已 toLowerCase 的标题 → 相关判定全部失效（详见"豁免/特殊逻辑"）。

### 错误级（阻断通过）

| 判定条件 | 错误消息 |
|---------|---------|
| 主标题含非 ASCII 中文字符（共性 C1；豁免：￡/™/罗马数字Ⅰ及Ⅱ-Ⅹ/白自在/至尊宝） | 主标题包含中文或中文字符 |
| 副标题为空（共性 C3） | 副标题为空 |
| 副标题含"动画"但分类≠405 Animations | 类型未选择Anime(动漫) |
| 分类/媒介/主视频编码/主音频编码/分辨率任一下拉未选（共性 C2） | 未选择分类／未选择媒介／未选择主视频编码／未选择主音频编码／未选择分辨率 |
| 标题解析出的媒介/编码/音频/分辨率与下拉不一致（共性 C2；**媒介比对因大写失效仅 remux/webdl/webrip/dvdrip/bdrip 形态生效**——原盘/UHD/WEB-DL 带连字符/HDTV 检测不出免比对，docs/38 §五；编码比对 AV1 检测 ID=12 不在常量表 → 误报"标题检测视频编码为undefined"） | 标题检测媒介为X，选择媒介为Y（视频编码/音频编码/分辨率同构共 4 条） |
| 官种 MI 未解析（**实际失效**：官种判定 `includes("HDK")` 大写匹配小写串恒 false） | 媒体信息未解析 |
| 官种未选制作组（同上失效） | 未选择制作组 |
| GodDramas 种 + 简介含"禁止转载" + 未选禁转标签 | 未选择禁转标签 |
| GodDramas 种 + cat≠421 Playlet/短剧（**麒麟短剧 ID=421**，非 AGSV/CS 的 419） | 未选择短剧类型 |
| GodDramas 种 + 未选驻站标签 | 未选择驻站标签 |
| 选了"官方"标签（**官种判定失效 → 非官种恒真，任何选官方标签的种子均报错**——HDK 特有缺陷） | 非官种不可选择官方标签 |
| 官种或 GodDramas 种未选"麒麟火"标签（官种侧失效，仅 GodDramas 生效；**HDK 特有**——AGSV"冰种"对应） | 未选择麒麟火标签 |
| MediaInfo 栏为空（共性 C4） | 请补充Mediainfo信息 |
| 简介无"片名/译名/名/演/主持人/简/国家"任一字段（**恒不报**：条件含单字"名/演/简"几乎恒真，docs/38 §五） | 未填写影片简介 |
| 简介内图片总数<2（共性 C5） | 缺少海报或截图 |
| cat=411 主标题缺 kHz／缺 bit（**ID 错位残留**：麒麟 411=software/软件（AGSV/CS 的 411=Music），实际约束的是软件类） | 主标题缺少采样频率／主标题缺少比特率 |
| 官种 x264/x265 词形（**死代码**：绑 category===6=HDK 组且官种判定失效） | 主标题中编码应为 x264／x265 |

### 警告级（提示不阻断）

| 判定条件 | 警告消息 |
|---------|---------|
| 分辨率=480p（麒麟 ID 8=480p/480i）且非 GodDramas/官种种 | 请检查是否有更高清的资源 |
| 标题含 complete + 未选"完结"标签 + cat∈{402 电视剧, 404 纪录教育}（**AGSV 为错误级，HDK 降级为警告**；硬编码的 403 综艺在麒麟不存在——实际综艺 420 不受检） | 完结剧集请添加完结标签 |
| 简介含 MediaInfo 参数图片（pterclub 模板图/文件名含 Mediainfo.png） | 请删除多余的影片参数/媒体信息图片 |
| 体积>1T（基本信息含 TB）未选"大包"标签 | 未选择大包标签 |

### 豁免/特殊逻辑

- **自动过审（HDK 特有）**：无错误时脚本 500ms 后自动点击"一键通过"按钮，2s 后置自动关页标记——其余站仅显示绿色"此种子未检测到错误"，HDK 直接过审动作
- **停用规则清单**（注释/未使用）：未信任组黑名单（错误级与警告级均注释）、MediaInfo 含 bbcode、分集标签、国语/中字/英字标签联动（三条全注释）、IMDb/豆瓣/TMDB 链接检查、**"简介中包含Mediainfo"**（isBriefContainsInfo 计算后未在任何规则中使用——HDK 允许 MI 入简介）
- **分类整体豁免**：cat∈{408 HQ Audio, 411 software, 412 Game, 413 Ebook, 418 Picture}（硬编码的 415 在麒麟分类表中不存在——空条件）→ 清空全部错误+警告；411 的 kHz/bit 在清空之后重加，仍生效
- 官种音乐清空逻辑（HDKMV）：大写匹配失效——死代码
- **大小写判定失效清单**（title 已 toLowerCase 仍匹配大写词，v1.3.15 实测）：`"HDK"`（官种）→ 媒体信息未解析/未选择制作组/官种未选官方标签/未选麒麟火(官种侧)/x264·x265 词形全部失效，且"非官种不可选择官方标签"反向恒报；`"HDKMV"`（音乐官种）；媒介分支 `"WEB-DL"`/`"REMUX"`(大写)/`"HDTV"`/`"DVD(原盘)"` → 媒介比对形同虚设
- **GodDramas 判定正常**（goddramas 小写）：禁转/短剧 421/驻站/麒麟火四条对短剧组生效

### 标题词形规范

- HDK 无词形硬规则；cat=411（软件，ID 错位残留）需含 `kHz`+`bit`
- **媒介标题检测实际生效分支仅**：`webdl`（无连字符）→WEB-DL；`remux`→Remux；`webrip`/`web-rip`/`dvdrip`/`bdrip`→Encode；其余（原盘/UHD/WEB-DL 带连字符/HDTV）检测不出 → 免比对
