...
// 不属于内容, 转发无意义。混排全剥（§59.284 架构模型：禁转判定收归 flags 通道，标题净化单责）。
func TestStripSiteOperationMarkers(t *testing.T) {
	cases := []struct{ in, want string }{
		...
		{"副标题\u00a0\u00a0 [2X 50%]", "副标题"},
		{"副标题 [Free 50%]", "副标题"},
		// §59.284: 禁转全剥（flags 通道单责——§59.136"剥促销保禁转"折中由架构模型取代）
		{"简繁双语特效四字幕\u00a0\u00a0\u00a0 [中性种子(NL)] [禁转]", "简繁双语特效四字幕"},
		{"副标题 [50%] [禁转]", "副标题"},
		{"副标题 [50%] [中性种子(NL)] [谢绝转载]", "副标题"},
		// [禁转] 单独存在: 同剥（§59.284）
		{"副标题 [禁转]", "副标题"},
	}
	for _, c := range cases {
