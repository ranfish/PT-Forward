...
**当前主线**：种子配置页数据源完善（六 Tab 逐项闭环）+ Token Registry 三阶段完成（§59.27）+ L2 搜索匹配链路修复（§59.26）。v0.0.587→v0.0.606 完成种子配置页操作列（获取/清除/重获）、管线统一（BuildTechProfile）、声明提取（栈解析器 + MediaInfo 拆分 + 致谢模板）、标签推断贯通（auto_feed 式多源）、Tab1 字段补全（声道/bit/中文名/类型中文标签）、L2 搜索四类失败修复（DTS-HD.MA 变体/CJK 降级重试/CSS 截断组名/sceneName 兜底）、禁转标记两层检测（跨词伪命中 + 定向禁转 + GORM 零值残留）。
