...
var (
	...
	reNexusBrowseAltLink  = regexp.MustCompile(`(?s)href=["'][^"']*\bdetails?\.php\?(?:id|group_id)=(\d+)[^"']*["'][^>]*>(.*?)</a>`)
	reNexusBrowseSize     = regexp.MustCompile(`(?i)class=["']?(?:rowfollow|[^"']*torrent-info-text-size)["']?[^>]*>([\d.]+)\s*(?:&nbsp;)?\s*(?:<br\s*/?>)?\s*(?:&nbsp;)?\s*(TiB|GiB|MiB|KiB|TB|GB|MB|KB|T|G|M)\b`)
