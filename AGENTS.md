...
**遗留**：无——发布页重构 grilling Round 3 六问全部定案（§59.124-130，2026-08-27），进入实施阶段（发布页独立新建+簇列表+TagApplier 按站灰度+team 域首批必做+老向导拆除）。**遗留清理六项全部完结（2026-08-27）**：①批量截图并发限流（§59.58 观察）②MarkStatus 订阅隔离（§59.120，v0.0.773）③runAnalyze 改判随发布页重构废弃（§59.121 发布零拉取定案）④E2E flaky 根治（§59.122 SQLite cache=shared，v0.0.774，顺带评分清理阈值配置化）⑤海报死链重触发（维持随发布页重构——当前数据零受影响+重取自愈，§59.119 观察项说明已补）⑥站点别名域名随版本同步（§59.123 migration 24，v0.0.775，243 实测 10/10）。
